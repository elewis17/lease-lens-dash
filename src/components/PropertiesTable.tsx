...
export default function PropertiesTable({ properties, onAdd, onUpdate, onDelete }: PropertiesTableProps) {
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50 border-b border-border">
              <TableHead className="px-4 py-2">Alias (unique)</TableHead>
          ...
          <TableBody>
                  ...
                  <>
                    <TableCell className="px-4 py-2">
                      <Input
                        ...
                        className="h-8"
                        placeholder="Short unique name (e.g., Maple-12)"
                      />
                    </TableCell>
                    <TableCell className="px-4 py-2">
                      <Input
                    ...
                    <TableCell className="px-4 py-2">
                      <div className="flex items-center gap-2">
                        <Input
                          type="number"
                          value={overrideTaxes ? (editData.property_taxes ?? "") : (editData.sale_price ? Math.round((editData.sale_price) * 0.013) : "")}
                          onChange={(e) => setEditData({ ...editData, property_taxes: e.target.value ? Number(e.target.value) : undefined })}
                          className="h-8"
                          disabled={!overrideTaxes}
                          placeholder="auto"
                        />
            ...
            {isAdding && (
              <TableRow className="bg-primary/5">
                <TableCell className="px-4 py-2">
                  <Input
                    placeholder="Unique alias (required)"
                    value={editData.alias ?? ""}
                    onChange={(e) => setEditData({ ...editData, alias: e.target.value })}
                    className="h-8"
                  />
